...
<script src="https://www.pixeep.com/wp-content/themes/pixeep/assets/frontend/js/instafeed.js?ver=1.0.0"></script>
<script>
	jQuery(document).ready(function(){

		jQuery('li.cart--link').click(function(e){
			e.preventDefault();
			jQuery('body').find('.site-header-cart').addClass('focus');
		});

		jQuery('.card-overlay,.mini-cart-header .close-icon').click(function(){
			jQuery('body').find('.site-header-cart').removeClass('focus');
		});
	//Instagram Feed
		if(typeof(instagramFeed) != 'undefined' && jQuery('body').find('.instagram_feed').length > 0 ){
